...
def analyze_american_law_dataset(base_path, max_files=10):
    ...
    if file_ids:
        ...
        if os.path.exists(html_file_path):
            ...
            for row in html_df.head(3).itertuples():
                print(f"  doc_id: {row['doc_id']}")
                print(f"  html_title: {row['html_title']}")
                print("  " + "-" * 40)
            
            # Check for linkage between citation, HTML, and embedding parquets
            citation_df = pd.read_parquet(os.path.join(base_path, f"{file_ids[0]}_citation.parquet"))
            embeddings_df = pd.read_parquet(os.path.join(base_path, f"{file_ids[0]}_embeddings.parquet"))
            print("\nCitation to HTML document linkage:")
            print(f"  Citation CIDs: {citation_df['cid'].nunique()} unique values")
            print(f"  Embeddings CIDs: {embeddings_df['cid'].nunique()} unique values")
            print(f"  HTML CIDs: {html_df['cid'].nunique()} unique values")
            
            # Check for overlap
            common_cids = set(
                citation_df['cid'].unique()
            ).intersection(
                set(html_df['cid'].unique())
            ).intersection(
                set(embeddings_df['cid'].unique())
            )
            print(f"  Common CIDs between citation, embeddings, and HTML: {len(common_cids)}")
            ...
            if len(citation_df) > 0:
                ...
                # Try to find matching HTML and embedding content for this citation
                example_cid = example['cid']
                matching_html: pd.DataFrame = html_df[html_df['cid'] == example_cid]
                matching_embeddings: pd.DataFrame = embeddings_df[embeddings_df['cid'] == example_cid]
                
                if not matching_html.empty and not matching_embeddings.empty:
                    print("\nMatching HTML, citation, and embedding content found for this citation:")
                    ...
                    print("\nNo matching HTML content and embedding found for this citation CID")
    ...
    print("Each municipality has three file types:")
    ...
    print("  3. Embedding files (_embeddings.parquet) - Contain vector representations of the legal text")
...
if __name__ == "__main__":
    ...
    base_path = "/home/ath/.cache/huggingface/hub/datasets--the-ride-never-ends--american_law/american_law/data"
